...
var ApiCall = function(name, specifier, steps, input) {
	...
	this._type = 'ApiCall';
	this.events = MetaEvents('{napi.apiCall}' + this._resource.name + '.' + this._method.name + '.' + this._specifier._name + '.' + name);
	// from parameters
	this._input = input;
	// calculated
	this._data = {};
	...
	return function() {
		...
		return Promise(
			function(chainResolve, chainReject, chainNotify) {
				...
				var result = q('apiCall.steps.init');
				list.unshift(
					{
				...
				list.forEach(
					function (listItem) {
						apiCall.events.metaEmit('nextStep', listItem);
						if (typeof listItem.rejectStep === 'function') {
							...
							return result = result.then(listItem.resolveHandler(apiCall)).catch(eHandler);
						}
					}
				);
				return result;
