...
DBWrapper.prototype.init = function() {	
	// console.log('DBWrapper ____ init');
	var thisDBWrapper = this;
	var url = 'mongodb://' + this._config.host + ':' + this._config.port + '/' + this._config.db;
	// Use connect method to connect to the Server
	return Promise(
		function(resolve, reject, notify) {
			// console.log('DBWrapper ____ connect');
			thisDBWrapper.initialconnect = false;
			MongoClient.connect(
				url, 
				function(err, db) {
					// console.log('DBWrapper ____ rdy');
					thisDBWrapper.db = db;
					thisDBWrapper.collection = null;
					if (err) {
						// console.log('DBWrapper ____ reject');
						return reject(err);
					}
					thisDBWrapper.initialconnect = true;
					thisDBWrapper.collection = db.collection(thisDBWrapper._config.collection);
					// console.log('DBWrapper ____ resolve');
					return resolve(thisDBWrapper.collection);
				}
			);
		}
	);
