...
var Specifier = function(name, parent, steps, parameters, tests) {
	...
	for (var n=0; n<steps.length; n+=1) {
		if (!steps[n]) {
			console.error('step ' + n + ' is not well-defined for specifier ' + thisSpecifier._name);
			continue;
		}
		layerName = steps[n][0];
		stepCall = steps[n][1];
...
Specifier.prototype.executor = function(document, options, callback, callOptions) {
	...
	this.set('executorParameters', parameters);
	this.set('executorDocument', document);
	...
	if (typeof callOptions.clientId === 'string') {
		...
		this.apiCall.set('clientId', callOptions.clientId);
	}
	this.on(
		'finished',
		function() {
	...
	if (typeof callOptions.pause === 'boolean' && callOptions.pause === true) {
		return thisSpecifier;
	}
	thisSpecifier._go(callOptions);
	return thisSpecifier;
};


Specifier.prototype._go = function(callOptions) {
	var document = this.get('executorDocument');
	var parameters = this.get('executorParameters');
	var parametersValid =  
		(
		...
		? 
			this._validateParameters(document) 
		: 
			true
	; 
	this.emit('run', parameters, this);
	return this._nextStep(parametersValid, null);
};
