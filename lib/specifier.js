...
Specifier.prototype._nextStep = function(err, results) {
	var thisSpecifier = this;
	var activeStepName = this._steps.getLast();
	var callPath = this.get('callPath');
	var next = null;
	...
	if (thisSpecifier._debug === true) {
		var e = this.apiCall.get('errors');
		var r = this.apiCall.get('results');
		if (r === null) r = [];
		...
		console.log('->[specifier debug @ ' + callPath + '.' + activeStepName + '] apiCall err, res.length -----------', e, r.length);
	}
